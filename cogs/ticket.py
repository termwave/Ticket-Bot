...
class tickredel(discord.ui.View):
    ...
    async def _treopen(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.channel.edit(name=f"ticket-{interaction.user.name}", overwrites=overwrites, category=cat, reason= f"Ticket Reopened by {str(interaction.user)} [{interaction.user.id}]")
        ...
        if p is not None:
            ...
            message = f"{user.mention} Welcome Back! The ticket is reopened"
        embed = discord.Embed(description="You will be provided with support shortly\nTo close this ticket click the 🔒 button.", color=0x7aaaff).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.avatar.url)
        v = ticketchannelpanel(self.bot)
        await interaction.channel.send(message, embed=embed, view=v)
        self.bot.add_view(v)
        await interaction.message.edit(view=None)

    @discord.ui.button(label="Delete", emoji="<:Delete:1291064244095160361>", custom_id="delete", style=discord.ButtonStyle.red)
    async def _delete(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            await user.send(content="Here's the transcript for your ticket:", file=transcript_file)
        except discord.Forbidden:
            await interaction.followup.send(f"Unable to send transcript to {user.mention}'s DMs.", ephemeral=True)  
        await deleteudata(guild, count, data)
        await interaction.followup.send("Deleting this ticket in 5 seconds.")
...
class TranscriptButton(discord.ui.View):
      def __init__(self, bot):
          super().__init__(timeout=None)
          self.bot = bot
      @discord.ui.button(label="Transcript", emoji="💬", custom_id="transcript", style=discord.ButtonStyle.grey)
      async def _transcript(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if transcript is None:
            await interaction.followup.send("Unable to export the transcript.", ephemeral=True)
            return
        transcript_file = discord.File(io.BytesIO(transcript.encode()), filename=f"transcript-{interaction.channel.name}.html")
        try:
            await interaction.user.send(content="Here's the transcript for your ticket:", file=transcript_file)
            await interaction.followup.send("Transcript sent to your DMs.", ephemeral=True)
        except discord.Forbidden:
            await interaction.followup.send("Unable to send transcript to your DMs. Please check your DM settings.", ephemeral=True)
...
class ticketchannelpanel(discord.ui.View):
    ...
    @discord.ui.button(label="Close", emoji="🔒", custom_id="close", style=discord.ButtonStyle.red)
    async def _ticketchannel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        u = TranscriptButton(self.bot)
        em = discord.Embed(title="🔒 Ticket Closed", description=f"{interaction.channel.mention} is closed by {str(interaction.user)}", color=0x7aaaff).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.avatar.url)
        await interaction.message.channel.send(embed=em, view=v)
        await interaction.message.channel.send(view=u)
        self.bot.add_view(v)
        self.bot.add_view(u)
        await interaction.message.edit(view=None)
...
class ticketpanel(discord.ui.View):
    ...
    @discord.ui.button(label="Create Ticket", emoji="<:nexus_Ticket:1291064396616826912>", custom_id="panel", style=discord.ButtonStyle.gray)
    async def _ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if srole:
            overwrites[srole] =  discord.PermissionOverwrite(view_channel = True, send_messages = True, attach_files = True, embed_links = True)
        channel = await interaction.guild.create_text_channel(name=f"ticket-{interaction.user.name}", overwrites=overwrites, category=cat, reason= f"Ticket Created for {interaction.user} [{interaction.user.id}]")
...
class rolemenuview(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction):
        if interaction.user.id != self.ctx.author.id and interaction.user.id not in  [1036877996243562516, 926831289649201213]:
...
class embmsg(discord.ui.Modal, title="Embed Message"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        des = self.emb.value + "\nTo create a ticket interact with the button below <:nexus_Ticket:1291064396616826912>"
        query = "SELECT * FROM  panel WHERE guild_id = ?"
...
######### BY AMAN ##########     
    ...
    async def on_ready(self):
      v = ticketchannelpanel(self.bot)
      st = ticketpanel(self.bot)
      self.bot.add_view(v)
      self.bot.add_view(st)
      print("Ticket system loaded succesfully")
############################     
    @commands.hybrid_group(name="ticket", description="Shows the help menu for ticket commands")
    async def ticket(self, ctx):
        prefix = ctx.prefix
        if prefix == f"<@{self.bot.user.id}> ":
            prefix = ctx.prefix

        ls = ["ticket", "ticket setup", "ticket reset", "ticket edit", "ticket info", "ticket reopen", "ticket close", "ticket transcript", "ticket rename"]
        des = ""
        for i in sorted(ls):
            cmd = self.bot.get_command(i)
            des += f"`{prefix}{i}`\n{cmd.description}\n\n"
        listem = discord.Embed(title=" Ticket Commands", colour=0x7aaaff,
    ...
    @ticket.command(description="Generates transcript of the current ticket.")
    async def transcript(self, ctx: commands.Context):
        v = TranscriptButton(self.bot)
        await ctx.send("Click the button to generate a transcript.", view=v)
    @ticket.command(description="Sends you close Button.")
    @commands.has_guild_permissions(administrator=True)
    async def close(self, ctx: commands.Context):
        v = ticketchannelpanel(self.bot)
        await ctx.send("To Close Click the button below.", view=v)
        
    @ticket.command(description="Renames the specified ticket.")
    ...
    async def rename(self, ctx: commands.Context, *, new_name: str):
        try:
            ...
            await ctx.send(f"An error occurred while renaming the channel: {e}")

    @ticket.command(description="Closes the current ticket and deletes the ticket after 5 seconds.")
    @commands.has_guild_permissions(administrator=True)
    async def delete(self, ctx: commands.Context):
        guild = ctx.guild
        channel = ctx.channel
        srole = await getsupportrole(guild)
        user = await getuser(guild, channel)
        ...
        if srole:
            overwrites[srole] = discord.PermissionOverwrite(view_channel=True, send_messages=True, attach_files=True, embed_links=True)
        await channel.edit(overwrites=overwrites, category=cat, reason=f"Ticket Closed by {str(ctx.author)}")
        data = [user.id, channel.id]
        await updateopendata(guild, int(count), data)
        await updatecloseddata(guild, int(count), data)
        await ctx.send("Deleting this ticket in 5 seconds.")
        await asyncio.sleep(5)
        await channel.delete(reason=f"Ticket Deleted by {str(ctx.author)} [{ctx.author.id}]")
    ...
    async def reopen(self, ctx: commands.Context, *, channel:discord.TextChannel):
        ...
        if srole:
            overwrites[srole] =  discord.PermissionOverwrite(view_channel = True, send_messages = True, attach_files = True, embed_links = True)
        await channel.edit(name=f"ticket-{interaction.user.name}", overwrites=overwrites, category=cat, reason= f"Ticket Reopened by {str(ctx.author)} [{ctx.author.id}]")
        data = [user.id, channel.id]
        await updateopendata(guild, int(count), data)
        await updatecloseddata(guild, int(count), data)
        p = await getpingrole(guild)
    ...
    @ticket.command(description="Setups a ticket panel for the server")
    @commands.has_guild_permissions(administrator=True)
    async def setup(self, ctx: commands.Context):
        if ctx.guild.owner.id == ctx.author.id:
            pass
        else:
            if ctx.author.top_role.position <= ctx.guild.me.top_role.position and ctx.author.id not in  [1036877996243562516, 926831289649201213]:
                em = discord.Embed(description=f"<:no:1275760052778831883> You must Have Higher Role than Bot To run This Command", color=0xff0000)
